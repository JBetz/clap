...
newtype PluginEntryHandle = PluginEntryHandle { unPluginEntryHandle :: Ptr C'clap_plugin_entry }
    deriving (Show)

clapEntry :: PluginLibrary -> IO PluginEntryHandle
clapEntry (PluginLibrary dl) = do
    clapEntryFunPtr <- dlsym dl "clap_entry"
    pure $ PluginEntryHandle (castFunPtrToPtr clapEntryFunPtr)

init :: PluginEntryHandle -> FilePath -> IO Bool
init (PluginEntryHandle pluginEntry) filePath = do
...
getFactory (PluginEntryHandle pluginEntry) pluginFactoryId = do
    ...
    withCString pluginFactoryId $ \cPluginFactoryId -> do
        ...
        pure $ if factory /= nullPtr
            then Just $ PluginFactoryHandle (castPtr factory)
            else Nothing
